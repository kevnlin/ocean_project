...
*Status 2026-08-08: **measured**. 4000 and 6000 profiles/month were run
(seed 1234) and the answer is **≈4050 profiles/month**. Rerun:
`python experiments/31_density_powerlaw.py`.*
...
| **4000** | **0.1006** | **0.0219** | **1** |
| **6000** | **0.0817** | **0.0184** | **1** |

> The 4000/6000 rows are **single-seed** — no ± is an error bar there, it is one
> measurement. Seeds 1235/1236 are still to run. For scale, the 3-seed spread at
> neighbouring densities is ±0.001–0.002 °C, so the conclusions below are not
> plausibly seed artifacts, but the numbers stay provisional.
>
...
**TEMP crosses 0.1 °C at ≈4050 profiles/month.** The measurement at 4000 is
0.1006 °C — 0.6 % above target — and the local slope past that point puts the
crossing at N ≈ 4044.

At 6000 the model reaches **0.0817 °C**, comfortably past the target, and SALT
reaches 0.0184 PSU against an advisor target of 0.04–0.05 that was already met
at every density on this curve.
...
Returns are still *accelerating* at 6000 profiles/month — the steepest segment
of the whole curve is the last one. Whatever irreducible error floor exists (the
...
remains the wrong model (R² = 0.966 with systematic residuals, −12 % at 100 and
−11 % at 6000), but the *direction* of its error was the opposite of what was
claimed.
...
Phase 4's [SSH ablation](ssh_ablation.md) reaches **0.1366 °C at 1500
profiles/month** (protocol_v1) by adding one derived channel, against 0.1586
without it. On this curve, buying that same 14 % improvement with profiles alone
would take roughly 1500 → 2800 profiles/month. A satellite-derived field is
...
the 0.1 °C crossing moves left of 4000.
...
Remaining: seeds 1235/1236 at 4000/6000 for a 3-seed headline, and the `mlp` /
`unet_joint` rows at those densities if the full method table is wanted (only
`clim_floor` and `unet_depthwise` were run, since the density curve is fitted on
those).
