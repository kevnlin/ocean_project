...
#
#   bash experiments/synthetic/run_seeds_queue.sh ssh     4     # arg: which queue, which GPU
...
for s in $SEEDS; do
  ...
  if [ "$QUEUE" = "ssh" ]; then
    $PY -u experiments/synthetic/29_ssh_ablation.py --seed "$s" \
    ...
    $PY -u experiments/synthetic/08_density_ablation.py --seed "$s" \
