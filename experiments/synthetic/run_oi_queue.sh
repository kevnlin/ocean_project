...
$PY -u experiments/synthetic/26_oi_tuning.py --split val > outputs/logs/oi_tuning_val.log 2>&1
echo "[$(date +%T)] stage 1 exit=$?"

echo "[$(date +%T)] stage 2/3: OI vs U-Net on the 12 pinned test months"
$PY -u experiments/synthetic/27_oi_vs_unet.py --verify-unet > outputs/logs/oi_vs_unet.log 2>&1
echo "[$(date +%T)] stage 2 exit=$?"

echo "[$(date +%T)] stage 3/3: tuning stability check on training months"
$PY -u experiments/synthetic/26_oi_tuning.py --split train > outputs/logs/oi_tuning_train.log 2>&1
